...
void HMWModule::processEventKey(){
   ...
   // "i-Message" ueber broadcast senden
     void HMWModule::sendInfoMessage(byte channel, unsigned int info, unsigned long target_address) {
  	   ...
  	   byte bSecondByte = info & 0xFF;

  	   byte bFirstByte = info >> 8;


  	   hmwrs485->txFrameData[2] = bFirstByte;//info / 0x100;
  	   hmwrs485->txFrameData[3] = bSecondByte;//info & 0xFF;
  	   hmwrs485->sendFrame();
     };
